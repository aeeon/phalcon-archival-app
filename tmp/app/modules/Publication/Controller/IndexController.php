...
class IndexController extends Controller {
    ...
    public function indexAction() {
        $type = 'artykuly'; //$this->dispatcher->getParam('type', 'string');
        $category = $this->dispatcher->getParam('category', 'string');
        $subcategory = $this->dispatcher->getParam('subcategory', 'string');
        $subsubcategory = $this->dispatcher->getParam('subsubcategory', 'string');
        ...
        $sscatModel = Category::getCachedBySlug($subsubcategory);
        ...
        if ($sscatModel) {
            $cat_id = $sscatModel->getId();
            $cond_array[] = "category_id = $cat_id";
            $title = $sscatModel->getTitle();
            $model = $sscatModel;
        } else if ($scatModel) {
        ...
        if ($model instanceof \Publication\Model\Type) {
            $publications = $model->getPublications(array("conditions" => "status='publish' AND date <= NOW()", "order" => "date DESC"));
        } else {
    ...
    AND p.status = 'publish'";
            ...
            $phql .= "GROUP BY p.id ORDER BY p.date DESC";

            $query = $this->modelsManager->createQuery($phql);

            $query->cache(
                    array(
                        "key" => md5($type . $model->getSlug()),
                        "lifetime" => 360
                    )
            );
            $publications = $query->execute();
            // print_r(count($publications->toArray()));exit;
        }
        //  echo $cat;exit;
        //   $publications = $model->getPublications(array("conditions"=>"status='publish' AND date <= NOW()", "order"=>"date DESC"));
        ...
        $this->view->cat = $cat;
        $this->view->dzial = $catModel->getId();
    ...
    public function publicationAction() {
        ...
        $cat = current($publication->getCategories(array("order" => "depth DESC"))->toArray());


        if (!$publication) {
            throw new \Phalcon\Mvc\Dispatcher\Exception("Publikacja '$slug.html' nie została znaleziona");
