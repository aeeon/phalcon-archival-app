$(function () {
    ...
    $('.save').click(function (e) {
        ...
        if (data) {
            $.post("/treemenu/admin/saveTree", {root: root, data: data}, function (response) {
                if (response.success == true) {
                    noty({layout: 'center', type: 'success', text: 'Element "'+root+'" zapisany', timeout: 2000});
                }
            }, 'json');
        }
    ...
    $('.add').click(function (e) {
        var root = $(this).data('root');
        var title = prompt("Wpisz tytuł nowej kategorii:", '');
        console.log(title);
        if (title) {
            $.post("/treemenu/admin/add", {root: root, title: title}, function (response) {
                if (response.success == true) {
                  ...
                  var info = $("<span>").addClass('info').html('(' + ')');
                    var edit = $("<a>").attr('href', '/treemenu/admin/edit/' + response.id)
...
function deleteCategory(category_id, node) {
    if (confirm('Czy napewno chcesz usunąć tą kategorię?')) {
        $.post('/treemenu/admin/delete', {category_id: category_id}, function (response) {
            if (response.success) {
