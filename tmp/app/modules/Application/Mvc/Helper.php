...
class Helper extends \Phalcon\Mvc\User\Component {
    ...
    public function isUriActive($pattern) {
        $uri = $this->url->path() . ltrim($this->router->getRewriteUri(), '/');
        if (stristr($uri, $pattern)) {
            return true;
        }
        return false;
    }
    ...
    public function alfabet() {;
        $array1 = range('A','L'); // tablica A-Z
        array_push($array1, "Ł");
        $array2 = range("M", "S");
        array_push($array2, "Ś");
        $array3 = range("T", "Z");
        array_push($array3, "Ź", "Ż");
        $array4 = array_merge($array1, $array2, $array3); // Wszystko razem
        //sort($array3, SORT_LOCALE_STRING);
        return $array4;
    }
    ...
    public function isRoleAllowed($module, $action) {
        $session = $this->getDi()->get('session');
        $auth = $session->get('auth');
        $acl = $this->getDi()->get('acl');
        if ($auth) {
            if ($acl->isAllowed($auth->role, $module, $action)) {
                return true;
            }
        }
        return false;
    ...
    public function getCatUrl($item) {
        echo $item->getTitle();
        $categories = $item->getCategories(array("order"=>"depth DESC"))->toArray();
        print_r($categories);exit;
        $cat = \Tree\Model\Category::getCachedById($id);
        $url = "";
        if ($cat) {
            
            $slug = $cat->getSlug();
            if ($cat->getParentId()) {
                $parent = \Tree\Model\Category::getCachedById($cat->getParentId());
                // $parent = \Tree\Model\Category::findFirst("id='{$cat->getParentId()}'");
                $url = $parent->getSlug() . "/";
            }

            $url .= $slug;
        }
        $urlObj = $this->getDI()->get('url');
        if (empty($url))
            return $urlObj->path() . $type;
        else
            return $urlObj->path() . $type . "/" . $url;
    }

    public function socialButtons($link, $class) {
        ?>
        <div class="<?php echo $class ?>">
            <ul>
                <li><a onClick="window.open('http://www.facebook.com/sharer.php?s=100&p&p[url]=<?php echo $link ?>', 'sharer', 'toolbar=0,status=0,width=580,height=325');" href="javascript: void(0)"><i class="fa fa-facebook"></i></a>
                </li>
                <li><a onClick="window.open('https://twitter.com/intent/tweet?url=<?php echo $link ?>', 'sharer', 'toolbar=0,status=0,width=580,height=325');" href="javascript: void(0)"><i class="fa fa-twitter"></i></a>
                </li>
