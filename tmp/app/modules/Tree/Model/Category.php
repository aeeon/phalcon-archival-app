...
class Category extends Model {
    ...
    public function initialize() {
        ...
        $this->hasManyToMany(
                "id", "Publication\Model\PublicationCategories", "category_id", "publication_id", "Publication\Model\Publication", "id", array('alias' => 'publications')
        );
    }
    ...
    public static function getListArray($root) {
        ...
        foreach ($result as $el) {
            ...
            foreach ($result_sub as $el_sub) {
                ...
                $list[$kklucz] = $vvalue;
                $result_sub_sub = self::find([
                            'conditions' => "depth = '3' AND parent_id='{$el_sub->getId()}'",
                            'order' => 'left_key',
                ]);
                foreach ($result_sub_sub as $el_sub_sub) {
                    $kkklucz = $el_sub_sub->getId();
                    $ident = "";
                    for ($i = 1; $i < $el_sub_sub->getDepth(); $i++)
                        $ident .= "&nbsp;&nbsp;&nbsp;&nbsp;";

                    $vvvalue = $ident . $el_sub_sub->getTitle();
                    $list[$kkklucz] = $vvvalue;
                }
            }
        }
